...
pbmc_data <- readRDS('./batchCorrected_matrix_clusterFltr_logNorm_log2.Rds')

pheno <- read_csv('./pheno_assgnmnt_logNorm.csv')
info <- read_csv('./clustering_metadata_logNorm.csv', guess_max = 100000)
...
regression.subset <- read_csv('./5.0.5_regression_CD8_Cluster_ME_Int_OneGrp_logNorm_subset.csv')
...
# LOAD DATA:
pbmc_counts <- Read10X('/data/TCR/10X_Genomics/data/interim/10X_PIPELINE_P1_P4/GEX/aggr_CR4-0-0_crossLongCombined_noNormalize/outs/filtered_feature_bc_matrix')
data <- readRDS('./batchCorrected_matrix_clusterFltr_logNorm_log2.Rds')
clusters <- read_csv('./clustering_metadata_logNorm.csv', guess_max = 100000)
pheno <- read_csv('./pheno_assgnmnt_logNorm.csv')
clusters <- clusters %>% inner_join(pheno)

regression <- read_csv('./5.0.5_regression_CD8_Cluster_ME_Int_OneGrp_logNorm_subset.csv')
regression$age_p_adj <- p.adjust(regression$age_p, method = 'BH')
#regression <- regression %>% filter(age_p_adj < 0.05, abs(co) > log2(1.1)/70)
  
mode.regression <- read_csv('7.0.2_Regression_RegressionValues10_0.05.subset_logNorm_integrated.csv')
mode.values <-read_csv('7.0.2_Regression_RegressionData10_0.05.subset_logNorm_integrated.csv')
...
# # Save Data:
# 
write.csv(score.subset, paste('./',version,'_Regression_RegressionValues',t_exptotall,'_',t_ptotal,'.cell_logNorm_integrated.csv', sep = ''), row.names = F)
write.csv(dfin2, paste('./',version,'_Regression_RegressionData',t_exptotall,'_',t_ptotal,'.cell_logNorm_integrated.csv', sep = ''), row.names = F)

# write.csv(score.subset, paste('/',version,'_Regression_RegressionValues',t_exptotall,'_',t_ptotal,'.subset.csv', sep = ''), row.names = F)
# write.csv(modes.of.change.subset[[2]], paste('./',version,'_Regression_RegressionData',t_exptotall,'_',t_ptotal,'.subset.csv', sep = ''), row.names = F)
# 
# write.csv(score.cell, paste('./',version,'_Regression_RegressionValues',t_exptotall,'_',t_ptotal,'.cell.csv', sep = ''), row.names = F)
# write.csv(modes.of.change.cell[[2]], paste('./',version,'_Regression_RegressionData',t_exptotall,'_',t_ptotal,'.cell.csv', sep = ''), row.names = F)
...
write.csv(gsea_split_all, paste('./',version,'_Modes_GSEAModesEnrichment_integrated_logNorm_subsets_10perc.csv',sep=''))
