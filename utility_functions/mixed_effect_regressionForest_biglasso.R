...
ME_LASSO  <- function(
  ...
  source('/gpfs/gsfs5/users/TCR/__SCRIPTS_Raheel/scripts-Project_Independent/machine_learning_functions.R')
  require(biglasso)
  require(tidyverse)
  require(MASS)
  require(Matrix)
  ...
  if( is.null(sigmasqzero) ) {
    sigmasqzero <- 1
  } else {
    sigmasqzero <- sigmasqzero
  }
  ...
  train_matrix <- as.matrix(MERF.lDB[,c(3:(ncol(MERF.lDB)-1))])
  fit.enet <- Generate_Multivariate_CrossValidation_v2(train_matrix, 
                                               labels_train = MERF.lDB$star.Yi,
  ...
  #fixed part
  #as vector
  oob <- as.matrix(fit.enet[[2]][,2:12])
  oob[is.na(oob)] <- 0
  MERF.lDB$f.pred<- rowSums(oob) # THese are the out of bag predictions!!!!
  ...
  #fixed part
  #as vector			
  #MERF.lDB$f.pred  <- fit.ranger$predictions # THese are the out of bag predictions!!!!
  
  train_matrix <- as.matrix(MERF.lDB[,c(3:(ncol(MERF.lDB)-2))])
  fit.enet <- Generate_Multivariate_CrossValidation_v2(train_matrix, 
                                               labels_train = MERF.lDB$star.Yi,
                                               model_type = model_type, 
                                               alpha = 0.5, 
